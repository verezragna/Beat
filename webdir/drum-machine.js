...
var soundName = document.getElementById('label');
...
for (var i = 0; i < data.tracks.length; i++) {
    saveObj[i] = fillLine(saveObj[i]);
}
console.log(saveObj);

document.getElementById("addLine").onclick = function() {
    data.tracks.push(createTrack("gold", note(audio, 330)));
    saveObj[data.tracks.length - 1] = fillLine(saveObj[data.tracks.length - 1]);
    console.log(saveObj);
}

function fillLine(massive) {
    massive = [];
    for (var i = 0; i < 16; i++) {
        massive[i] = 0;
    }
        ...
        data.tracks.forEach(function(track, row) {
            ...
            track.steps.forEach(function(on, column) {
                ...
                if (isPointInButton(p, column, row)) {
                    ...
                    if (track.steps[column] === true) {
                        console.log(saveObj);
                        saveObj[row][column] = 1;
                    } else saveObj[row][column] = 0;
                }
            });
        });
    });
...
document.getElementById('save').onclick = function() {
    localStorage.setItem(soundName, JSON.stringify(saveObj));
    soundName.value = "";
}

document.getElementById('open').onclick = function() {
    var saveObj = JSON.parse(localStorage.getItem(soundName));
    data.tracks.forEach(function(track, row) {
        ...
        track.steps.forEach(function(on, column) {
            ...
            if (saveObj[row][column] == 1) {
                track.steps[column] = true;
            } else track.steps[column] = false;
        });
    });
    soundName.value = "";
}
